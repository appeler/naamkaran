...
from naamkaran._resources import HF_REPO, HF_REVISION, resolve_model
...
def test_missing_artifact_uses_exact_pinned_location(
    ...
    monkeypatch.setenv("NAAMKARAN_MODEL_DIR", str(tmp_path))
    with patch(
        "huggingface_hub.hf_hub_download", return_value="/cache/model.pt"
    ) as download:
        assert resolve_model("naamkaran.pt") == "/cache/model.pt"
